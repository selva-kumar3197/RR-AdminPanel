...
function InsightList() {
  ...
  const handleDeleteSubmit = () => {
    ...
    axios
      ...
      .then((res) => {
        if (res.status == 200) {
          toast.success("Successfully Deleted Insight");
          setLoader(false);
  ...
  const handleValidation = () => {
    ...
    const { image } = file;
    if (!heading) {
      newErros.heading = "please enter Insight Heading";
    } else if (heading && !textReg.test(heading)) {
      newErros.heading = "numbers is not allowed";
    } else if (heading && heading.length > 50) {
      newErros.name = "Insight should be below 50 charecters";
    }

    if (!text) {
      newErros.text = "please enter Insight text";
    } else if (text && !textReg.test(text)) {
      newErros.text = "numbers is not allowed";
    } else if (text && text.length > 50) {
      newErros.text = "Insight text should be below 50 charecters";
    }
    if (!image) {
    ...
    axios({
      ...
      .then((res) => {
        if (res.status == 200) {
          setReloadPage(reloadPage + 1);
          setShow(false);
          toast.success("Successfully Updated insights");
          setLoader(false);
          setupdateMemberForm({
            ...updateMemberForm,
            name: "",
  ...
  const [colDefs, setColDefs] = useState([
    ...
    {
      headerName: "Insight Heading",
      filter: true,
      field: "title",
    },
    {
      headerName: "Insight Text",
      filter: true,
  ...
  return (
    <div>
      ...
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Update Insight</Modal.Title>
        </Modal.Header>
        <Modal.Body>
