...
function CaseStudyList() {
  ...
  const productsListApi = () => {
    axios({
      mathod: "GET",
      url: `${baseURL}/category/casestudy`,
    })
      .then((res) => {
        if (res.status == 200) {
  ...
  const handleDeleteSubmit = () => {
    ...
    axios
      ...
      .then((res) => {
        if (res.status == 200) {
          toast.success("Successfully Deleted casestudy");
          setLoader(false);
  ...
  const handleValidation = () => {
    ...
    const newErros = {};
    const { heading, text, name } = updateMemberForm;
    const { image } = file;
    if (!heading) {
      newErros.heading = "please enter casestudy Heading";
    } else if (heading && !textReg.test(heading)) {
      newErros.heading = "numbers is not allowed";
    } else if (heading && heading.length > 100) {
      newErros.heading = "casestudy should be below 50 charecters";
    }
    ...
    if (!text) {
      newErros.text = "please enter casestudy text";
    } else if (text && !textReg.test(text)) {
      newErros.text = "numbers is not allowed";
    } else if (text && text.length > 100) {
      newErros.text = "casestudy text should be below 100 charecters";
    }
  ...
  const handleSubmit = () => {
    ...
    formData.append("category", "casestudy");
    ...
    axios({
      ...
      .then((res) => {
        if (res.status == 200) {
          setReloadPage(reloadPage + 1);
          setShow(false);
          toast.success("Successfully Updated casestudy");
          setLoader(false);
          setupdateMemberForm({
            ...updateMemberForm,
            name: "",
  ...
  return (
    <div>
      ...
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Update case study</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          ...
          <div className="container ">
            ...
            <div className="row my-2">
              <div className="col-md-3">
                <h3>
                  <b>All case study List </b>
                </h3>
              </div>

              <div className="col-md-4 text-end">
                <button type="button" className="btn btn-outline-success mx-1">
                  <Link to="/casestudy-add">Add Case Study </Link>
                </button>
                <button
                  type="button"
                  ...
                  disabled
                >
                  <Link to="/casestudy-list"> Case Study List</Link>
                </button>
              </div>
            </div>
            {/* <table className="table table-bordered w-100 my-2 text-center">
                            <thead>
                                <tr className="col-12" >
                                    ...
                                    <th className="col-3">Blog Heading</th>
                                    <th className="col-1" >Blog Text</th>
                                    <th className="col-1" >Blog Image</th>
