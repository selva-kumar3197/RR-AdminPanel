...
function ServicesList() {
  ...
  const handleValidation = () => {
    ...
    const { image } = file;
    if (!heading) {
      newErros.heading = "please enter service Heading";
    } else if (heading && !textReg.test(heading)) {
      newErros.heading = "numbers is not allowed";
    } else if (heading && heading.length > 50) {
      newErros.name = "service should be below 50 charecters";
    }

    if (!text) {
      newErros.text = "please enter service text";
    } else if (text && !textReg.test(text)) {
      newErros.text = "numbers is not allowed";
    } else if (text && text.length > 50) {
      newErros.text = "service text should be below 50 charecters";
    }
    if (!image) {
