...
function BlogList() {
  ...
  const handleValidation = () => {
    ...
    const { image } = file;
    if (!heading) {
      newErros.heading = "please enter article Heading";
    } else if (heading && !textReg.test(heading)) {
      newErros.heading = "numbers is not allowed";
    } else if (heading && heading.length > 50) {
      newErros.name = "article should be below 50 charecters";
    }

    if (!text) {
      newErros.text = "please enter article text";
    } else if (text && !textReg.test(text)) {
      newErros.text = "numbers is not allowed";
    } else if (text && text.length > 50) {
      newErros.text = "article text should be below 50 charecters";
    }
    if (!image) {
  ...
  const handleSubmit = () => {
    ...
    formData.append("category", "pr_news");
    ...
    if (updateMemberForm.report_date) {
      formData.append("report_date", updateMemberForm.report_date);
    }
    if (updateMemberForm.link) {
      formData.append("link", updateMemberForm.link);
    }
    ...
    // },
    axios({
      url: `${baseURL + 'pr_news/'}${updateID.id}`,
      method: "PUT",
      data: formData,
    })
      .then((res) => {
            ...
            <div className="row my-2">
              ...
              <div className="col-md-4 text-end">
                ...
                <button
                  ...
                  disabled
                >
                  <Link to="/blog-list"> PrNews List</Link>
                </button>
              </div>
            </div>
