...
function ExecutivesList() {
  ...
  const productsListApi = () => {
    axios({
      mathod: "GET",
      url: `${baseURL}/category/executives`,
    })
      .then((res) => {
        if (res.status == 200) {
  ...
  const handleValidation = () => {
    ...
    const newErros = {};
    const { heading, text, role } = updateMemberForm;
    const { image } = file;
    if (!heading) {
  ...
  const handleSubmit = () => {
    ...
    if (updateMemberForm.role) {
      formData.append("role", updateMemberForm.role);
    }
            ...
            <Row className="mb-6">
              <Form.Group as={Col}>
                ...
                <Form.Control
                  ...
                  placeholder="Role"
                  name="role"
                  value={updateMemberForm.role}
                  onChange={handleChange}
                  autoComplete="off"
                />
                <span className="text-danger">
                  {updateMemberFormErrors.role}
                </span>
              </Form.Group>
            </Row>
